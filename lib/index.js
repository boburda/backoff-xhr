...
exports.client = function(method, handleSuccess, handleError) {
    ...
    this.retries = 0;
    this.maxRetries = 4;
    this.setParams = inParams => {Object.assign(this.params, inParams)};
    ...
    retryHandler = (res) => {
        if(this.retries < this.maxRetries) {
            ...
            xhr.open(this.getMethod(), this.getUrl());
            setTimeout(()=> { xhr.send(requestBody);
            }, backoffTime);
            this.retries+=1;
        } else {
            this.reject(res);
     ...
     * */
    var validationHandler = function() { return this.url && this.method && xhr.status <= 1 };
    /**
     * @access public
    ...
    this.exec = async function() {
        ...
        if(unsetHandler) {
            ...
            if(xhr.readyState !== xhr.OPENED) xhr.open(this.method, this.url);
	...
	xhr.onreadystatechange = e => {
        if(this.checkDone()) {
            ...
            if(this.successOn.includes(xhr.status)) {
                this.autoResolve ? this.resolve(xhr.responseText) : successHandler.call(this, xhr.responseText, xhr.status)
                ...
                console.log("rejected", xhr.responseText); 
                this.autoReject ? this.reject(xhr.responseText) : errorHandler.call(this, xhr.responseText, xhr.status)
            } 
...
exports.rp = (url, options)=> {
    var client = new exports.request();
